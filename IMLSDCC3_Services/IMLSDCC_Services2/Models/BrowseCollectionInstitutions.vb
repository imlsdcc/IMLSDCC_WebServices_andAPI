...
Public Class BrowseCollectionInstitutions
    ...
    Public Sub New(ByVal id As String, ByVal startsWith1 As String, ByVal startRecord1 As Integer, ByVal maximumRecords1 As Integer)
        ...
        'Dim rscount = (From r In db.CollectionProperties Where r.property = "publisher" And r.text.StartsWith(startsWith1) Order By r.text Select r.text).Distinct
        Dim rs = (From r In db.CollectionProperties Join i In db.CollectionInstitutions On i.collectionID Equals r.collectionID Where r.property = "publisher" And r.text.StartsWith(startsWith1) Order By r.text Select r.text).Distinct
        'Dim rs = (From r In db.CollectionProperties Where r.property = "publisher" And r.text.StartsWith(startsWith1) Order By r.text Select r.text).Distinct
        Dim xcount = rs.Count
        totalRecords = xcount
        If rs.Count = 0 Then
        ...
        Else
            ...
            For Each j In rs.Skip(startRecord1).Take(maximumRecords1)
                Dim iname = (From ci In db.CollectionInstitutions Join c In db.CollectionProperties On c.collectionID Equals ci.collectionID Where c.text = j Select ci.institutionID).Take(1)
                Dim tempitem As New items("/Search/Collection/institution=" & iname.First & "&startRecord=1&maximumRecords=10", j)
