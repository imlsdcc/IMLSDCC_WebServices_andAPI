...
Public Class SearchDates
    ...
    Public Sub New(ByVal searchargs1 As String, ByVal wheretosearch As String, ByVal startRecord1 As Integer, ByVal maximumRecords1 As Integer, ByVal scope1 As String, ByVal sort1 As String)
        ...
        uriThisPage = "/Search/Item?date=" & searchargs1 & "&iStartRecord=" & startRecord1 & "&iMaximumRecords=" & maximumRecords1 & "&scope=" & scope1 & "&sort=" & sort1
        uriNextPage = "/Search/Item?date=" & searchargs1 & "&iStartRecord=" & startRecord1 + maximumRecords1 & "&iMaximumRecords=" & maximumRecords1 & "&scope=" & scope1 & "&sort=" & sort1
        uriPreviousPage = "/Search/Item?date=" & searchargs1 & "&iStartRecord=" & startRecord1 - maximumRecords1 & "&iMaximumRecords=" & maximumRecords1 & "&scope=" & scope1 & "&sort=" & sort1

        Dim ret = (From r In db.Records Join r2 In db.RecordsToFacets On r.recordID Equals r2.recordID Join f In db.Facets On f.facetID Equals r2.facetID Where datelist.Contains(r2.facetID) And f.facetType = "Date" Select r.title, r.recordID)
        ...
        If Not ret Is Nothing Then
            ...
            For Each j In ret
                Dim rs = (From r In db.Records Where r.recordID = j.recordID Select r.title)
                Dim z = rs.Single
                Dim tempitem As New items("/Item/Detail/" & j.recordID, Left(z, 127))
